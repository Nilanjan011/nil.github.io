    ...
    if (empty($_REQUEST['name']) || empty($_REQUEST['phone']) || empty($_REQUEST['email']) || empty($_REQUEST['dob']) || empty($_REQUEST['address']) || empty($_REQUEST['gender'])) {
        ...
        if (isset($id) && $id!='') {
            if (isset($tmp_img) && $tmp_img!="") {
                ...
                $sql = "UPDATE `rrr` SET `name`='$name',`email`='$email',`phone`='$phone',  `dob`='$dob',`address`='$address',`gender`='$gender',`file`='$image'   WHERE `id`='$id'";
                if(mysqli_query($conn, $sql)){
                    $sql="SELECT * FROM `rrr` WHERE `id`=$id";
                    $res=mysqli_query($conn,$sql);
                ...
                $sql = "UPDATE `rrr` SET `name`='$name',`email`='$email',`phone`='$phone',  `dob`='$dob',`address`='$address',`gender`='$gender' WHERE `id`='$id'";
                if(mysqli_query($conn, $sql)){
